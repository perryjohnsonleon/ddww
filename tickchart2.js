...
// ── RENDER ─────────────────────────────────────────────────────────────────
 async function renderMain() {
	  ...
	  const post = await getData(stockId_list[0]);
	  if (post) {		  
 ...
 async function renderMarkets() {
  ...
  if (post) {		  
	...
	row.innerHTML = `
	  ...
	  <div class="market-price-col" id="mprice-0">$${m.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
